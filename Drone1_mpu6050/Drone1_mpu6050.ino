...
void setup() {
  ...
  Serial.println("Kalibrerar gyro...");
  for (int i = 0; i < 2000; i++) {
    if (!read_mpu_6050_data()) {
      Serial.println("MPU read fail → Recovery");
      i2cReinit();
      setup_mpu_6050_registers();
      i--;           // försök igen på samma index
      delay(3);
      continue;
    }

    // SUMMERA råa gyro-värden (för bias)
    gyro_x_cal += gyro_x;
    gyro_y_cal += gyro_y;
...
// ... (setup och alla andra delar exakt som innan)

void loop() {
  // ⏱️ Mät faktisk tid mellan loopar
static uint32_t tPrev = micros();
uint32_t tNow = micros();
float dt = (tNow - tPrev) * 1e-6f;  // sekunder
tPrev = tNow;

// Om dt är orimligt (t.ex. vid I2C-timeout), tvinga till 4 ms
...
angle_pitch_gy += (gyro_x / 65.5f) * dt;  // °/s * s = °
angle_roll_gy  += (gyro_y / 65.5f) * dt;

  // Stabilare beräkning av acc-vinkel
  angle_roll_acc  = atan2((float)acc_y, (float)acc_z) * 57.296f;
  angle_pitch_acc = atan2(-(float)acc_x,
                        sqrt((float)acc_y*(float)acc_y + (float)acc_z*(float)acc_z)) * 57.296f;
...
void setup_mpu_6050_registers() {
  ...
  // --- Nytt: slå på digitalt lågpassfilter (DLPF) ---
  Wire.beginTransmission(0x68);
  Wire.write(0x1A);   // CONFIG
  Wire.write(0x05);   // DLPF ~42 Hz gyro / 44 Hz accel
  Wire.endTransmission();
}
